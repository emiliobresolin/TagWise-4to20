...
const migrations: DatabaseMigration[] = [
  ...
  },
  {
    ...
    apply: async (database, _now) => {
      await database.execAsync(`
        CREATE TABLE IF NOT EXISTS user_partitioned_execution_evidence (
          owner_user_id TEXT NOT NULL,
          work_package_id TEXT NOT NULL,
          tag_id TEXT NOT NULL,
          template_id TEXT NOT NULL,
      ...
      await database.execAsync(`
        CREATE INDEX IF NOT EXISTS idx_user_partitioned_execution_evidence_owner
        ON user_partitioned_execution_evidence (
          owner_user_id,
          work_package_id,
          tag_id,
          updated_at DESC
  ...
  },
  {
    ...
    apply: async (database, _now) => {
      ...
      const reportedAtColumn = await database.getFirstAsync<{ count: number }>(
        `
      ...
      if ((reportedAtColumn?.count ?? 0) === 0) {
        await database.execAsync(`
          ALTER TABLE mobile_runtime_error_events
      ...
      await database.execAsync(`
        ...
        ON mobile_runtime_error_events (reported_at, captured_at ASC);
      `);
    },
  },
];
